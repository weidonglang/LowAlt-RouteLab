...
from app.core.graph_builder import iter_neighbors
...
def plan_dijkstra(
    ...
    end_grid: str,
    level: str,
    options: DijkstraOptions | None = None,
) -> PlanResult:
    options = options or DijkstraOptions()
    started_at = perf_counter()

    validation_error = validate_plan_request(map_index, start_grid, end_grid, level)
    if validation_error:
        return build_failure_result(AlgorithmName.DIJKSTRA, validation_error, started_at)
    ...
    distance_by_state: dict[tuple[str, int | None], float] = {start_state: 0.0}
    ...
    while open_heap:
        ...
        for neighbor in iter_neighbors(map_index, current, level, options.allow_diagonal):
            ...
            tentative = distance_by_state[current_state] + edge_cost(
                map_index,
                current,
                neighbor,
            ...
            if tentative >= distance_by_state.get(neighbor_state, float("inf")):
                continue

            came_from[neighbor_state] = current_state
            distance_by_state[neighbor_state] = tentative
            sequence += 1
            heappush(open_heap, (tentative, sequence, neighbor_state))
